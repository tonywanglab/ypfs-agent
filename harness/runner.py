...
def run_case_samples(
    ...
    """Run one case N times and persist each sample.

    `history` and `agent_kwargs` are pass-throughs to agent.run() for callers
    that need a conversational run or an extended toolset (harness.chat).
    Omitting both is the eval path: empty history, the default tool registry.
    Every sample of a run replays the SAME history — samples are independent
    draws of one turn, not a conversation.
    """
