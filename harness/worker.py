...
# Backoff for database errors in the claim loop, doubling up to the cap.
QUEUE_ERROR_BACKOFF_S = 5.0
QUEUE_ERROR_BACKOFF_MAX_S = 60.0
...
# The chat UI is an optional feature package (harness/chat/). It owns the
# chat_turn / chat_regenerate kinds, so it registers their executors here rather
# than having this module know how a conversation works.
#
# The guard is narrow on purpose. A blanket `except ImportError: pass` would also
# swallow a real breakage inside harness/chat/ (a typo, a circular import) and
# leave the worker silently unable to run chat tasks — it would claim them and
# fail them with "Unknown task kind", which looks like a queue problem rather
# than an import problem. So: tolerate the package being absent, and let any
# other import failure raise.
try:
    import harness.chat.runs as _chat_runs
except ModuleNotFoundError as exc:  # pragma: no cover - only when harness/chat/ is removed
    if (exc.name or "").split(".")[:2] != ["harness", "chat"]:
        raise
else:
    _chat_runs.register_executors(EXECUTORS)
...
def worker_loop(name: str, stop_event: threading.Event) -> None:
    db_backoff = QUEUE_ERROR_BACKOFF_S
    while not stop_event.is_set():
        try:
            tasks.requeue_stale()
            task = tasks.claim(name)
        except Exception as exc:
            # Throw the connection away rather than retrying on it: a severed
            # connection can be left holding an unconsumed result, and then every
            # subsequent query on it fails identically forever.
            print(f"[{name}] queue error: {exc}")
            dbio.reset_conn()
            # Exponential backoff. A flat retry against an unreachable database
            # burns local ephemeral ports until the host runs out of them, which
            # takes down the whole process rather than just this loop.
            stop_event.wait(db_backoff)
            db_backoff = min(db_backoff * 2, QUEUE_ERROR_BACKOFF_MAX_S)
            continue
        db_backoff = QUEUE_ERROR_BACKOFF_S
        if task is None:
