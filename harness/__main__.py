...
def _cmd_chat_init(_args: argparse.Namespace) -> None:
    from .chat import schema as chat_schema

    chat_schema.init_chat_schema()
    tables = chat_schema.installed_tables()
    print(f"Chat schema applied ({len(tables)} tables): {', '.join(tables)}")
...
def main() -> None:
    ...
    sub.add_parser("chat-init",
                   help="Apply the chat UI schema (migrations/chat/001). Idempotent; "
                        "does not touch schema.sql's tables")
