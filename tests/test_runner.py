...
from harness import config, runner, seed
...
def test_run_case_samples_reports_progress(pg, monkeypatch):
    ...
    assert [phase for phase, *_rest in sample_one] == ["agent", "sample_done"]
    sample_two = [item for item in seen if item[1] == 2]
    assert [phase for phase, *_rest in sample_two] == ["agent", "sample_done"]


def test_delete_run_removes_run_and_samples(pg, monkeypatch):
    _seed_prompt()
    _patch_pipeline(monkeypatch)
    manifest = runner.run_case(Case("case_1", "q"), "prompt", "prompt_v1")
