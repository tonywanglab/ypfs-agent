...
def _client(pg, monkeypatch, role="admin"):
    """A test client for the eval console.

    The console is admin-only now (the chat UI's auth.install_guards default-
    denies everything outside the user-facing chat routes), so these tests sign
    in. Pass role="user" to assert on what an unauthenticated session sees.
    """
    seed.seed_all()
    monkeypatch.setenv("OPENROUTER_API_KEY", "test")
    monkeypatch.setenv("HARNESS_ADMIN_PASSWORD", ADMIN_PASSWORD)
    client = web.create_app().test_client()
    if role == "admin":
        response = client.post("/login", data={"password": ADMIN_PASSWORD})
        assert response.status_code == 302, "admin sign-in failed"
        # Drop sign-in's own flash so tests can assert on _flashes[0] being the
        # message their request produced.
