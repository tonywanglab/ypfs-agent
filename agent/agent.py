...
def _post(messages: list[dict], model: str, tools: list[dict]) -> dict:
...
def _call(messages: list[dict], model: str, tools: list[dict]) -> dict:
    ...
    for attempt in range(MAX_RETRIES + 1):
        try:
            return _post(messages, model, tools)
...
def run(
    ...
    `tools`/`dispatch_fn` let a caller extend the toolset for one run without
    mutating the module-level registry — same shape as
    harness.llm.run_tool_loop's dispatch_fn. Both default to tools.TOOLS /
    tools.dispatch, so callers that pass neither get exactly today's behavior.
    A tool the caller doesn't include is simply absent from the schemas sent to
    the model, which is how the harness keeps its admin-only tool off non-admin
    runs. Pass them together: a dispatch_fn must be able to serve every name in
    `tools`.
    ...
    try:
        for _ in range(MAX_STEPS):
            msg = _call(messages, model, tools)
            messages.append(msg)
            ...
            for c in calls:
                ...
                result = dispatch_fn(c["function"]["name"], args, context)
                messages.append({
                    "role": "tool",
                    "tool_call_id": c["id"],
